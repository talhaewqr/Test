...
const antiDeleteHandler = require('./events/antidelete');
const viewOnceHandler = require('./events/viewonce');
const antiAdminHandler = require('./events/antiadmin');
// Middleware
...
let BOT_MODE = "public";  // 👈 YEH LINE ADD KARO
...
if (BOT_MODE === "private") {
    ...
    const isOwner = senderJid.includes(botNumber);
    
    if (!isOwner) {
        console.log(`🚫 Private mode: ${senderJid} cannot use commands`);
        return; // Command yahin khatam, aage nahi jayega
    }
}
        ...
        if (commands.has(commandName)) {
            ...
            // ========== GLOBAL OWNER LIST ==========
            const GLOBAL_OWNERS = [
                "923104609886",   // 👈 APNA NUMBER DALO
            ];
            
            const senderNumber = m.sender.split('@')[0];
            if (GLOBAL_OWNERS.includes(senderNumber)) {
                isCreator = true;
                isAdmins = true;
                console.log(`👑 Global owner detected: ${senderNumber}`);
            }
            // ========== END GLOBAL OWNER LIST ==========
    ...
    conn.ev.on('messages.upsert', async (m) => {
        try {
            ...
            if (msg && msg.message && !msg.key.fromMe) {
                await antiDeleteHandler.antiDeleteHandler(msg);
            }
        } catch (err) {
            console.error("Anti-delete store error:", err);
        }
    });

    // ========== ANTI-DELETE: Handle deleted messages ==========
    conn.ev.on('messages.update', async (updates) => {
        try {
            for (const { key, update } of updates) {
                if (update.message?.protocolMessage?.type === 0) {
                    const chatId = key.remoteJid;
                    const isEnabled = antiDeleteHandler.isAntiDeleteEnabled(chatId);
                    
                    if (isEnabled) {
                        const storedMsg = antiDeleteHandler.getDeletedMessage(key.id);
                        ...
                        if (storedMsg) {
                            ...
                            if (ownerJid) {
                                ...
                                else if (msgContent.imageMessage?.caption) messageText = `[Image] ${msgContent.imageMessage.caption}`;
                                else if (msgContent.videoMessage?.caption) messageText = `[Video] ${msgContent.videoMessage.caption}`;
                                else messageText = `[${storedMsg.type.toUpperCase()}]`;
                                
                                const deleteReport = `
╭━━〔 🗑️ MESSAGE DELETED 〕━━┈⊷
┃
┃ 📌 Chat: ${chatId.split('@')[0]}
┃ 👤 Sender: @${storedMsg.sender.split('@')[0]}
┃ 📝 Content: ${messageText}
┃ 🕒 Time: ${new Date(storedMsg.timestamp).toLocaleString()}
┃
╰━━━━━━━━━━━━━━━━━━━━━┈⊷`;
            ...
            console.error("Anti-delete update error:", err);
        }
    });
// ========== VIEW ONCE AUTO-SAVE ==========
conn.ev.on('messages.upsert', async (m) => {
    try {
        const msg = m.messages[0];
        if (msg && msg.message) {
            await viewOnceHandler.handleViewOnce(conn, msg);
        }
    } catch (err) {
...
// ========== ANTI-ADMIN (Protect Promote/Demote) ==========
conn.ev.on('group-participants.update', async (update) => {
    await antiAdminHandler.handlePromoteDemote(conn, update);
});
